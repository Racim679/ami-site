...
// We'll use the first locality image or a default image
const cityImages: Record<string, string> = {
  "ALGER": babElOuedImage, // Default image for Alger
  "TIPAZA": heroImage, // Default image for Tipaza
};

// Map locality names to their corresponding images (from Localites.tsx)
  ...
  useEffect(() => {
    const fetchCitiesWithPropertyCounts = async () => {
      try {
        ...
        const citiesWithCounts: City[] = await Promise.all(
          (citiesData || []).map(async (city) => {
            // Get all localities for this city
            const cityLocalities = (localitiesData || []).filter(
              (locality) => locality.city_id === city.id
            );

            // Count properties in all localities of this city
            let totalCount = 0;
            for (const locality of cityLocalities) {
              const { count } = await supabase
                ...
                .neq('status', 'Vendu'); // Only count available properties
              totalCount += count || 0;
            }

            // Get image for city - use first locality image or default
            let cityImage = cityImages[city.name.toUpperCase()] || heroImage;
            if (cityLocalities.length > 0) {
              const firstLocality = cityLocalities[0];
              cityImage = localityImages[firstLocality.name] || cityImage;
            }
        ...
        const citiesWithProperties = citiesWithCounts
      ...
      <div className="text-center mt-4">
        <div className="w-full flex justify-center px-4 sm:px-0">
          <div className="w-full sm:w-auto max-w-xs sm:max-w-none">
            <AuditButton 
              text="Voir toutes les localités" 
              showArrow={true} 
