  ...
  useEffect(() => {
    const fetchCitiesWithPropertyCounts = async () => {
      try {
        ...
        } = await supabase.from("localities").select("*").order("name");
        ...
        const citiesWithCounts: City[] = await Promise.all((citiesData || []).map(async city => {
          ...
          const cityLocalities = (localitiesData || []).filter(locality => locality.city_id === city.id);
          ...
          for (const locality of cityLocalities) {
            const {
              ...
              count: 'exact',
              head: true
            }).eq('locality_id', locality.id).neq('status', 'Vendu'); // Only count available properties
            totalCount += count || 0;
          }
