...
const CRM = () => {
   ...
   const createDefaultPropertyStructures = async (propertyId: string) => {
      try {
         await supabase.from("property_amenities_structured").insert([{
            property_id: propertyId, piscine: false, garage: false, jardin: false,
            terrasse: false, balcon: false, cave: false, grenier: false, buanderie: false
         }]);

         await supabase.from("property_security_structured").insert([{
            property_id: propertyId, gardien: false, video_surveillance: false, alarme: false,
            digicode: false, interphone: false, portail_electrique: false, ascenseur: false, acces_handicape: false
         }]);

         await supabase.from("property_documents_structured").insert([{
            property_id: propertyId, titre_propriete: false, acte_propriete: false, livret_foncier: false,
            certificat_inscription_fonciere: false, plans_cadastraux: false, documents_cadastraux: false,
            fiche_fiscale: false, certificat_urbanisme: false, permis_construire: false,
            certification_conformite: false, contrat_location: false, promesse_vente: false,
            mainlevee: false, permis_exploitation: false, certificat_non_negativite: false, certification_possession: false
         }]);

         await supabase.from("property_nearby_structured").insert([{
            property_id: propertyId, ecoles: false, pharmacies: false, mosquees: false,
            transports_publics: false, banques: false, universites: false, commerces: false,
            restaurants: false, aeroports: false, hopitaux: false, parcs: false, plages: false
         }]);
      } catch (error) {
   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
         ...
         setFormData({
            title: "", description: "", status: "À Vendre", surface: "", price: "",
            typology: "", commune_id: "", image_url: "", latitude: "", longitude: "", phone_whatsapp: "+213",
         });
   ...
   const handleUpdate = async (e: React.FormEvent) => {
      ...
      try {
         ...
         toast({ title: "Succès", description: "Le bien a été modifié avec succès" });
   ...
   const addGalleryPhoto = async (url: string) => {
      ...
      try {
         const { data, error } = await supabase.from('property_photos').insert({ property_id: propertyId, text: url }).select().single();

         if (error) throw error;
         setGalleryPhotos(prev => [...prev, { id: data.id, url: url }]);
         toast({ title: "Photo ajoutée", description: "La photo a été ajoutée à la galerie" });
      } catch (error: any) {
         toast({ title: "Erreur", description: error.message || "Impossible d'ajouter la photo", variant: "destructive" });
      }
   };
   ...
   const getAlertCount = () => {
      ...
      count += properties.filter(p => {
         ...
         return differenceInDays(new Date(), new Date(p.updated_at)) > 30;
      }).length;

      // Incomplete
      count += properties.filter(p => calculatePropertyHealth(p).score < 60).length;
            ...
            <div className="max-w-6xl mx-auto">
               ...
               <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  ...
                  <TabsContent value="sales">
                     <SalesAnalytics properties={properties} />
                  </TabsContent>
                           ...
                           <form onSubmit={editingProperty ? handleUpdate : handleSubmit} className="space-y-6">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                 ...
                                 <div className="space-y-2">
                                    ...
                                    <Select value={formData.typology} onValueChange={(value) => handleInputChange("typology", value)}>
                                 ...
                                 <div className="space-y-2">
                                    ...
                                    <Select value={formData.commune_id} onValueChange={(value) => handleInputChange("commune_id", value)}>
                                       <SelectTrigger><SelectValue placeholder="Sélectionner une commune" /></SelectTrigger>
                                       <SelectContent>
                                          {communes.map((commune) => (
                                             <SelectItem key={commune.id} value={commune.id.toString()}>{commune.name}</SelectItem>
                                          ))}
                                       </SelectContent>
                                    </Select>
                                 </div>
                                 ...
                                 <div className="space-y-2 md:col-span-2">
                                    ...
                                    {formData.image_url && (
                                       <div className="mt-2">
                                          <img src={formData.image_url} alt="Aperçu" className="h-20 w-20 object-cover rounded-lg border" />
                                       </div>
                                    )}
                                 </div>
                                             ...
                                             <div className="grid grid-cols-4 gap-2">
                                                {galleryPhotos.map((photo) => (
                                                   <div key={photo.id || photo.url} className="relative group">
                                                      <img src={photo.url} alt="Photo galerie" className="h-20 w-full object-cover rounded-lg border" />
                                                      <Button
                                                         variant="destructive"
                                                         size="sm"
                              ...
                              {newPropertyId && (
                                 <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                                    ...
                                    <Button onClick={() => setActiveTab("details")} className="w-full" variant="default">
                        ...
                        <CardContent>
                           {editingProperty || newPropertyId ? (
                              ...
                              <div className="text-center py-8 text-muted-foreground">
                                 ...
                                 <p className="text-sm mb-4">Pour accéder aux détails avancés :</p>
